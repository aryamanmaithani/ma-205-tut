...
\begin{frame}{Greetings}
    ...
    \end{enumerate}
    \uncover<8->{You can find a link to this document on \url{bit.ly/ma-205}. Both with and without pauses. You may keep it open alongside for quick reference. }
\end{frame}
\begin{frame}{Warning}
...
\begin{frame}{Lecture 2: CR Equations}
    \uncover<1->{\begin{thm}[CR equations]
        ...
        \end{equation*} }
    \end{thm} }
    \uncover<4->{Note the subscript is $x$ for both in the above. }\\
    \uncover<5->{Also note that all the equalities are only at {\color{red}the point $z_0.$} }\uncover<6->{In particular, we are only assuming differentiability at $z_0.$ }
\end{frame}
\begin{frame}{Lecture 2: CR Equations}
    Converse? What is the converse? \uncover<2->{{\color{red}No.} The converse is {\color{red}not} true. }\uncover<3->{An example for you to check is
    \begin{equation*} 
...
\begin{frame}{Lecture 2: CR Equations}
    ...
    \uncover<4->{Thus, we have seen two sufficient conditions for complex differentiability so far. Neither is individually sufficient. }\uncover<5->{However, together, they are. }
\end{frame}
\begin{frame}{Lecture 2: CR Equations}
    \begin{thm}[]
        Let $f:\Omega\to\mathbb{C}$ be a function and let $z_0 \in \mathbb{C}.$ If \\
...
\begin{frame}{Lecture 3: Power Series}
    ...
    \uncover<4->{\begin{rem}
        ...
        \uncover<5->{The limsup might be $\infty.$ This is also allowed. }
    \end{rem} }
\end{frame}
\begin{frame}{Lecture 3: Power Series}
...
\begin{frame}{Lecture 4: Exponential function}
    ...
    \uncover<4->{\begin{thm}[Zeroes are isolated]
        Let $\Omega$ be a {\color{red}domain} and $f:\Omega\to\mathbb{C}$ be a {\color{red}non-constant} holomorphic function. \uncover<5->{Let $z_0 \in \Omega$ be such that $f(z_0) = 0.$ }\uncover<6->{Then, there exists $\delta > 0$ such that $f$ has no other zero in $B_\delta(z_0).$ }
    \end{thm} }
...
\begin{frame}{Lecture 8: Singularities}
    ...
    \end{defn}
    \uncover<4->{\begin{thm}[]
        An isolated singularity $z_0$ is a pole of $f$ iff
        \begin{equation*} 
    ...
    \uncover<5->{\begin{thm}[Order of a pole]
        ...
        \begin{equation*} 
            f(z) = (z - z_0)^{-m}f_1(z)
        \end{equation*} }\uncover<6->{on a punctured neighbourhood of $z_0,$ for some function $f_1$ which is holomorphic on the complete neighbourhood. }
    \end{thm} }
    \uncover<7  ->{If $m = 1,$ then $z_0$ is said to be \emph{simple} pole. }
\end{frame}
\begin{frame}{Lecture 8: Singularities}
