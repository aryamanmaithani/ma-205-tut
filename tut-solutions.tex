...
\begin{enumerate}
	...
	\begin{soln} \phantom{hi}\\	
		...
		Finally, this gives
		\begin{equation*} 
			v = \dfrac{1}{2}Y^2 + 3XY^2 - \dfrac{1}{2}X^2 - X^3.
		\end{equation*}
	\end{soln}
	%\\
	%\\
	...
	\begin{soln}
		...
		(So, not only did we show that $L > 1$ doesn't imply {\color{blue}divergence} but also that even $L = \infty$ is not good enough to conclude divergence.)
	\end{soln}
	%
	%
	%
	...
	\begin{soln}
		...
		However, since $\mathbb{C}$ is path-connected, this implies that $f$ is zero \emph{everywhere}, as desired.
	\end{soln}
	%\\
	%\\
	%
