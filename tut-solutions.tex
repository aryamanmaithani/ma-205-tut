...
\begin{enumerate}[leftmargin=*]
    ...
    \begin{soln}
    	...
    	As in the previous question, argue that the second integral goes to $0$ in the limit $R \to {\color{myupdatecolor}\infty}.$ Also, note that the first integral is exactly what we want, in the limit $R \to \infty.$ (The imaginary part will be zero because odd function.)
...
This is the last tutorial. This was a fun course for me. Hopefully, you felt the same.\\
We saw many nice theorems in this course. Things we didn't have in the case of $\mathbb{R}$eal differentiable functions. (Can you venture a guess why this is so? The definitions are so similar, after all.)

What were some good theorems? Well, we had CIF, holomorphic $\iff$ analytic, ODAD, Liouville, CRT, MMT.\\
We also saw some strong theorems which we didn't prove. Weierstrass product theorem, Little Picard, Mittag-Leffler and OMT.\\
There were also some little theorems that made life simpler. CR equations, ZAI, Identity theorem, RRST.\footnote{Can you name all theorems listed?}

Towards the end, we saw the power of CRT in computing real integrals. Wasn't that neat?

\doublerulefill

\begin{enumerate}
	\item Evaluate $\displaystyle\int_{0}^{2\pi} \dfrac{\cos^2(3x)}{5 - 4\cos(2x)} {\mathrm{d}}x.$
	\begin{soln}
		The idea is to convert the integral into over the unit circle. Note that $2\cos(n\theta) = z^n + z^{-n}$ for $z = e^{\iota\theta}.$ Thus, we manipulate the integral as follows:
		\begin{align*} 
			\int_{0}^{2\pi} \dfrac{\cos^2(3\theta)}{5 - 4\cos(2\theta)} {\mathrm{d}}\theta &= \dfrac{1}{4}\int_{0}^{2\pi} \dfrac{(2\cos(3\theta))^2}{5 - 2(2\cos(2\theta))} {\mathrm{d}}\theta\\
			&= \dfrac{1}{4}\int_{0}^{2\pi} \dfrac{(e^{3\iota\theta} + e^{-3\iota\theta})^2}{5 - 2(e^{2\iota\theta} + e^{-2\iota\theta})} {\mathrm{d}}\theta\\
			&= \dfrac{1}{4}\int_{0}^{2\pi} \dfrac{(e^{3\iota\theta} + e^{-3\iota\theta})^2}{5 - 2(e^{2\iota\theta} + e^{-2\iota\theta})}\dfrac{\iota e^{\iota\theta}}{\iota e^{\iota\theta}} {\mathrm{d}}\theta\\
			&= \dfrac{1}{4}\int_{|z| = 1}^{} \dfrac{(z^3 + z^{-3})^2}{5 - 2(z^2 {\color{myupdatecolor}+} z^{-2})}\dfrac{1}{\iota z} {\mathrm{d}}z\\
			&= -\dfrac{1}{8\iota}\int_{|z| = 1}^{} \dfrac{(z^6 + 1)^2}{z^5(z^4 - 5z^2/2 + 1)} {\mathrm{d}}z
		\end{align*}
		Note that $z^4 - 5z^2/2 + 1 = 0$ can be solved easily since it is a quadratic in $z^2.$ We get the solutions to be $\pm\sqrt{2}, \pm\dfrac{1}{\sqrt{2}}.$ Thus, we get all the poles as:
		\begin{equation*} 
			0, \pm \dfrac{1}{\sqrt{2}}, \pm \sqrt{2}.
		\end{equation*}
		Note that $\pm\sqrt{2}$ are outside the curve and thus, don't bother us. We now calculate the residue at all other poles.
		
		\hrulefill
		
		\textbf{Residue at $0$:} This is a pole of order $5.$ What a nightmare. Computing the fourth derivative is not something I would want to do. Let us compute the Laurent series directly.
		\begin{align*} 
			\dfrac{(z^6 + 1)^2}{z^5(z^4 - 5z^2/2 + 1)} &= \dfrac{1}{z^5}\dfrac{(z^6 + 1)^2}{1 - (5z^2/2 - z^4)}\\
			&= \dfrac{1}{z^5}(z^6 + 1)^2\left[1 + \left(\dfrac{5z^2}{2} - z^4\right) + \left(\dfrac{5z^2}{2} - z^4\right)^2 + \cdots\right]\\
			&= \dfrac{1}{z^5}\left\{[z^{12} + 2z^6 + 1]\left[1 + \left(\dfrac{5z^2}{2} - z^4\right) + \left(\dfrac{5z^2}{2} - z^4\right)^2 + \cdots\right]\right\}
		\end{align*}
		The desired residue is the coefficient of $1/z$ in the above, which is the coefficient of $z^4$ in $\{\cdots\}.$ The latter is seen to be
		\begin{equation*} 
			-1 + \dfrac{25}{4} = \dfrac{21}{4}.
		\end{equation*}
		
		\hrulefill
		
		\textbf{Residue at $1/\sqrt{2}:$} Note that $1/\sqrt{2}$ is a {\color{red}simple} pole. Thus, the residue calculation will also be {\color{red}simple}.\footnote{This is why these poles are called ``simple.''}
		...
		Thus, we get the residue to simply be
		\begin{align*} 
			\dfrac{\left(\left(\dfrac{1}{\sqrt{2}}\right)^6 + 1\right)^2}{\left(\left(\dfrac{1}{\sqrt{2}}\right)^2 - 2\right)\left(\dfrac{1}{\sqrt{2}} + \dfrac{1}{\sqrt{2}}\right)} &= \dfrac{\left(\dfrac{1}{8} + 1\right)^2}{\left(\dfrac{1}{\sqrt{2}}\right)^5\left(\dfrac{1}{2} - 2\right)\left(\dfrac{2}{\sqrt{2}}\right)}\\
			&= - \dfrac{\dfrac{81}{64}}{\left(\dfrac{1}{4\sqrt{2}}\right)\left(\dfrac{3}{2}\right)(\sqrt{2})}\\
			&= - \dfrac{27}{8}
		\end{align*}
		...
		Thus, we get the integral to be
		\begin{align*} 
			-\dfrac{1}{8\iota}\cdot2\pi\iota\left(\dfrac{21}{4} - \dfrac{27}{8} - \dfrac{27}{8}\right) &= -\dfrac{1}{8\iota}\cdot2\pi\iota\left(\dfrac{21}{4} - \dfrac{27}{4}\right)\\
			&= -\dfrac{1}{8\iota}\cdot2\pi\iota\left(-\dfrac{6}{4}\right)\\
			&= \boxed{\dfrac{3\pi}{8}.}
		\end{align*}
	\end{soln}
	%
	%
	\item Evaluate $\displaystyle\int_{|z - 2| = 4}^{} \dfrac{2z^3 + z^2 + 4}{z^4 + 4z^2} {\mathrm{d}}z.$
	\begin{soln}
		Define
		\begin{equation*} 
			f(z) \vcentcolon= \dfrac{2z^3 + z^2 + 4}{z^4 + 4z^2}.
		\end{equation*}

		The singularities of $f$ are $0, \pm 2\iota.$ Being only finitely many, they are all isolated. Moreover, all of them are poles. (Since none of them is a root of the numerator, they are not removable.) All of these lie within the given circle. Thus, the given integral is just 
		\begin{equation*} 
			2\pi\iota\sum_{z \in \{0, \pm2\iota\}}^{}\Res(f; z).
		\end{equation*}

		Let us now compute the residues. Before that, we just factor $f$ better as
		\begin{equation*} 
			f(z) = \dfrac{2z^3 + z^2 + 4}{z^2(z + 2\iota)(z - 2\iota)}.
		\end{equation*}
		
		\hrulefill
		
		\textbf{Residue at $0$:} It is clear that $0$ is a pole of order $2.$ Thus, the pole is given by $\dfrac{1}{1!}g'(0)$ where $g(z) \vcentcolon= z^2f(z).$ Thus, we have
		\begin{equation*} 
			g(z) = \dfrac{2z^3 + z^2 + 4}{z^2 + 4}.
		\end{equation*}
		Putting some hard effort, we see that
		\begin{equation*} 
			g'(z) = \dfrac{(z^2 + 4)'(2z^3 + z^2 + 4) - (z^2 + 4)(2z^3 + z^2 + 4)'}{(z^2 + 4)^2}.
		\end{equation*}
		Evaluating at $0,$ we get
		\begin{equation*} 
			g'(0) = \dfrac{0\cdot4 - 4\cdot0}{4^2} = 0.
		\end{equation*}
		Thus, we get $\Res(f; 0) = 0.$
		
		\hrulefill
		
		\textbf{Residue at $2\iota$:} Note that $2\iota$ is a {\color{red}simple} pole. Thus, the residue calculation will also be {\color{red}simple}.\footnote{The last footnote was a joke.} 

		The residue is simply given by $\displaystyle\lim_{z\to 2\iota}(z - 2\iota)f(z).$ In this case, we get that as
		\begin{align*} 
			\lim_{z\to 2\iota}(z - 2\iota)f(z) &= \lim_{z\to 2\iota}\dfrac{2z^3 + z^2 + 4}{z^2(z + 2\iota)}\\
			&= \dfrac{2(2\iota)^3 + 0}{(2\iota)^2(2\iota + 2\iota)}\\
			&= \dfrac{-16\iota}{-4(4\iota)}\\
			&= 1.
		\end{align*}
		Thus, we have $\Res(f;2\iota) = 1.$
		
		\hrulefill
		
		\textbf{Residue at $-2\iota$:} Similar calculation as earlier gives the residue here to be $1$ as well. (Can you argue this directly by taking conjugates? Without any explicit calculations?)

		\hrulefill
		
		Thus, we finally get the integral to be 
		\begin{equation*} 
			2\pi\iota\sum_{z \in \{0, \pm2\iota\}}^{}\Res(f; z) = 2\pi\iota(0 + 1 + 1) = \boxed{4\pi\iota}.
		\end{equation*}
	\end{soln}
	%
	%
	\item Show with and without the open mapping theorem that if $f$ is a holomorphic function on a domain $\Omega$ with $|f|$ is constant, then $f$ is constant. 
	\begin{soln}\phantom{hi}\\
		\textbf{Without OMT:} Writing $f = u + iv$ as usual, we see that 
		\begin{equation*} 
			u^2 + v^2 \equiv c.
		\end{equation*}
		...
		\textbf{With OMT:} Suppose $f$ is not constant. We show that this gives a contradiction.

		By our assumption that $f$ is not constant, OMT tells us that the image $f(\Omega)$ must be open in $\mathbb{C}.$ However, $|f|$ being constant tells us that $f(\Omega)$ is a subset of the circle $\{z : |z| = c\},$ where $c$ is the constant $|f|$ equals.

		However, no subset of such a circle is open. (Why?) This shows that $f$ must be constant.
	\end{soln}
	%
	%
	\item Show that $\displaystyle\int_{-\infty}^{\infty} \dfrac{x}{(x^2 + 2x + 2)(x^2 + 4)} {\mathrm{d}}x = -\dfrac{\pi}{10}.$
	\begin{soln}
		The usual technique. Define 
		\begin{equation*} 
			f(z) \vcentcolon= \dfrac{z}{(z^2 + 2z + 2)(z^2 + 4)}.
		\end{equation*}
		Its poles are $-1 \pm \iota, \pm 2\iota.$ Thus, if we take $R > 2,$ then all the poles \textbf{in the upper half plane} are enclosed in the following contour.
    	...
    	Applying residue theorem to the above contour to get
    	\begin{equation*} 
    		\int_{\gamma_1}^{} f(z) {\mathrm{d}}z + \int_{\gamma_2}^{} f(z) {\mathrm{d}}z = 2\pi\iota\sum_{z \in \{2\iota, -1+\iota\}}^{}\Res(f; z).
    	\end{equation*}

    	Along $\gamma_1,$ the integral is what we want, in the limit $R \to \infty.$ Along $\gamma_2,$ a simple application of ML tells us that the integral is zero, in the limit $R \to \infty.$ Thus, we get the desired integral to be
    	\begin{equation*} 
    		2\pi\iota\sum_{z \in \{2\iota, -1+\iota\}}^{}\Res(f; z).
    	\end{equation*}
    	Both the poles being considered above are simple and the residue calculation is simple. Just do it.
	\end{soln}

	\emph{Remark.} In the above calculation, we implicitly assumed that the desired integral \emph{did} exist and then we calculated it using a limit of the form $\displaystyle\lim_{R\to \infty}\int_{-R}^{R}.$\\
	However, this method did not actually show the existence of the improper integral. (There, the lower and upper limits tend to $\pm\infty$ independently.) However, it can be shown that if the integral is a rational function of the form $\dfrac{p(x)}{q(x)}$ such that $q$ has no real zeroes and $\deg q - \deg p \ge 2,$ then the integral does exist.\\
	({\color{myupdatecolor}This part does not require complex analysis. Just some real analysis.} Showing this is not too tough, you may ask me if you are interested and don't get it on your own.)
	%
	%
	\item Show that any injective entire function is of the form $az + b$ for some $a \neq 0.$
	\begin{soln}
		First we show that the function is a polynomial. From that, we will show that the function is linear.

		Since $f$ is entire, we can write
		\begin{equation*} 
			f(z) = \sum_{n = 0}^{\infty} a_nz^n
		\end{equation*}
		which is valid for all $z \in \mathbb{C}.$

		Consider the domains $\Omega_1 = \{z \in \mathbb{C} : |z| < 1\}$ and $\Omega_2 = \{z \in \mathbb{C} : |z| > 1\}.$

		Note that $f$ is not constant and thus, by the open mapping theorem, $f(\Omega_1)$ is open.

		Moreover, $\Omega_1\cap\Omega_2$ is empty and hence, injectivity forces that $f(\Omega_1)\cap f(\Omega_2) = \emptyset.$

		Note that $a_0 = f(0) \in f(\Omega_1).$

		\begin{blockquote}
		
		Now, assume that $a_n \neq 0$ for infinitely many $n.$ Then, $0$ is an essential singularity of $z\mapsto f\left(\dfrac{1}{z}\right).$ 

		Consider the complex number $a_0 \in \mathbb{C}.$ By Casorati-Weierstrass, there exists a sequence $z_n \to 0$ such that
		\begin{equation*} 
			f\left(\dfrac{1}{z_n}\right) \to a_0.
		\end{equation*}

		For $N$ large enough, we have that $|z_n| < 1$ for all $n \ge N.$ (Why?)\\
		Thus, we see that $\dfrac{1}{z_n} \in \Omega_2$ for $n \ge N.$

		Since $f(\Omega_1)$ is open, we may choose $\epsilon > 0$ such that $B_\epsilon(a_0) \subset f(\Omega_1).$ By definition of limit, we may find $N' \ge N$ such that
		\begin{equation*} 
			f\left(\dfrac{1}{z_n}\right) \in B_\epsilon(a_0)
		\end{equation*}
		for all $n \ge N'.$ However, this then tells us that $1/z_{N'} \in \Omega_2$ as well as $f(1/z_{N'}) \subset f(\Omega_1)$ giving us that
		\begin{equation*} 
			f\left(\dfrac{1}{z_{N'}}\right) \in f(\Omega_1)\cap f(\Omega_2)
		\end{equation*}
		and thus,
		\begin{equation*} 
			f(\Omega_1)\cap f(\Omega_2) \neq \emptyset,
		\end{equation*}
		a contradiction.

		\end{blockquote}
		
		Thus, we must have that $a_n \neq 0$ for only finitely many $n$ and thus, $f$ is a polynomial. Write
		\begin{equation*} 
			f(z) = \sum_{n = 0}^{N}a_nz^n
		\end{equation*}
		with $a_N \neq 0$ for some $N \ge 1.$ (Since $f$ cannot be constant.)

		\begin{blockquote}
			Assume that $N > 1.$ Then, FTA tells us that $f$ has $N > 1$ zeroes. Injectivity forces them to be all equal. Thus, $f(z) = c(z - z_0)^N$ for some $c \in \mathbb{C}^\times.$ However, $f(z) = 1$ then has $N$ distinct solutions, contradicting injectivity.
		\end{blockquote}

		Thus, we get $N = 1$ which proves the result.
	\end{soln}
