...
\begin{enumerate}
	...
	Compare this with the previous question.
	%
	\item Suppose that the domains in the previous question was replaced by an arbitrary domain $\Omega$ such that $\{n^{-1} : n \in \mathbb{N}\} \subset \Omega.$\\
	Characterise $\Omega$ precisely such the above $f(1/n) = 0$ condition would ensure that $f$ is constant. (That is, come up with a rule such that if $\Omega$ follows that rule, then $f$ has to be constant and that if $\Omega$ does not follow the rule, then $f$ may be non-constant.)\\
	...
	\end{equation*}
	(The LHS means that the function $f'/f$ is evaluated at $1/n$ and similarly foe the RHS.)\\
