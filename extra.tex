...
\begin{enumerate}
	...
	Let $f : \mathbb{D} \to \mathbb{C}$ be a holomorphic function. (Recall $\mathbb{D}$ from \Cref{sec:notations}.) \\
	...
	\end{align}
	Moreover, if equality holds in \Cref{eq:01} for some $z \in \mathbb{D} \setminus \{0\}$, or if equality holds in \Cref{eq:02}, then $f(z) = \lambda z$ for some $\lambda \in \mathbb{C}$ with $\md{\lambda} = 1$.
	\end{lem}
	...
	\begin{enumerate}
		...
		\end{equation*}
		\item Let $r \to 1$ to conclude \Cref{eq:01} and \Cref{eq:02}. (Hint for \Cref{eq:02}: What is $g(0)$?)
	...
	\begin{enumerate}
		...
		\item Observe that this solves our problem by showing that equality can actually be achieved in \Cref{eq:03}. \\
	...
	\begin{enumerate}
		...
		\end{equation*}
		Use \Cref{eq:04} to conclude that equalities hold above. \\
